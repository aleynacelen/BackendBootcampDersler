...
public class CartManager : ICartService
{
    ...
    public async Task<ResponseDto<CartDto>> CreateCartAsync(string applicationUserId)
    {
        ...
        // var  existsCart = await _cartRepository
    ...
    public async Task<ResponseDto<CartDto>> GetCartAsync(string applicationUserId)
    {
        try
        {
            ...
            // var cart = await _cartRepository.AddAsync();
            // X => X.ApplicationUserId==applicationUserId,
            // query => query.Include(x => x.CartItems).ThenInculude(y=>y.Product);
            // if(cart==null){
            //     return ResponseDto<CartDto>.Fail("kullanıcıya it sepet yok", StatusCodes.Status400BadRequest);
            // }
        ...
        catch (System.Exception)
        {
            ...
            // return Response<CartDto>.Fail(ex.Message);
        }
    }
    ...
    //    try
    //    {
    //     // var cartItem= await _cartItemRepository.GetAllAsync(cartItemId);
    //     if(cartItem==null){
    //         return ResponseDto<NoContent>.Fail("ilgili ürün sepettte bulunmadı",StatusCodes.Status400BadRequest);

    
    //     }
    //     if (results <1)
    //     {
    //         return ResponseDto<NoContent>.Fail(StatusCodes.sTATUS)
    //     }
    //    }
    //    catch (System.Exception)
    //    {
